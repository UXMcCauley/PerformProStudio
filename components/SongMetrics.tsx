...
export default function SongMetrics({ song, lyrics }: Props) {
  ...
  const renderCalendar = () => {
    ...
    return (
      <div className="bg-white/70 backdrop-blur-sm border border-slate-200/50 rounded-2xl p-4 md:p-6 shadow-lg shadow-purple-500/10">
        ...
        <div className="grid grid-cols-7 gap-1 md:gap-2">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
            <div key={day} className="text-center text-xs md:text-sm font-semibold text-slate-600 p-1 md:p-2">
              {day}
            </div>
          ))}
          {days.map((day, index) => {
              ...
              <button
                ...
                className={`p-1 md:p-2 text-xs md:text-sm rounded-xl transition-all duration-300 ${
                  isCompleted
                    ...
                    : 'bg-white/80 text-slate-900 hover:bg-gradient-to-br hover:from-purple-50 hover:to-pink-50 hover:text-purple-700 hover:scale-105 hover:shadow-md'
                }`}
              >
                {day.getDate()}
              </button>
    ...
    return (
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg shadow-purple-500/10 border border-slate-200/50 p-4 md:p-6">
        ...
        <div className="text-center py-12">
          ...
          <p className="text-slate-500">Select a song to view metrics</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg shadow-purple-500/10 border border-slate-200/50 p-4 md:p-6">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="bg-white/70 backdrop-blur-sm border border-slate-200/50 rounded-2xl p-4 shadow-lg shadow-purple-500/10">
          {lyrics.length === 0 ? (
            <div className="text-center py-8">
              <svg className="w-12 h-12 text-slate-300 mx-auto mb-3" fill="none" stroke="currentColor" strokeWidth="1" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
              <p className="text-slate-500">No lyrics added yet</p>
            </div>
          ) : (
            <>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {lyrics.map(line => {
                  ...
                  return (
                    <div key={line.id} className="group flex flex-col md:flex-row md:items-center gap-2 p-3 bg-white/60 rounded-xl border border-slate-200/50 hover:bg-white/80 hover:shadow-md transition-all duration-300">
                      <div className="flex-1 text-xs md:text-sm text-slate-900 truncate font-medium">
          ...
          {sessions.length === 0 ? (
            ...
            <div className="space-y-2">
              {sessions.map(session => (
                <div
                  key={session.id}
                  className="group flex flex-col md:flex-row md:items-center md:justify-between gap-2 p-3 bg-white/80 rounded-xl border border-slate-200/50 hover:bg-white hover:shadow-md transition-all duration-300 hover:scale-[1.02]"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full" />
                    <div className="text-xs md:text-sm text-slate-900 font-medium">
